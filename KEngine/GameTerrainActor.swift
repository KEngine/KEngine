...
class GameTerrainActor:GameActor {
    ...
    var m_weightTexture:MTLTexture! = nil
    ...
    init(scene:GameScene,m0:float2,m1:float2,m2:float2,m3:float2,depth:Int) {
        ...
        do{
            m_weightTexture = try m_scene.m_utility.m_textureLoader.newTextureWithContentsOfURL(NSBundle.mainBundle().URLForResource("weight", withExtension: "png")!, options: nil)
        }catch let error as NSError{
            fatalError(error.localizedDescription)
        }

        setupGeometryState()
        m_textureArray = GameTextureArray(textureFilePaths: ["grass1","grass2","rock","ground"], scene: m_scene)
    ...
    override func renderWithPipelineStates(encoder: MTLRenderCommandEncoder, pipelineState: MTLRenderPipelineState, depthState: MTLDepthStencilState) {
        ...
        encoder.setFragmentTexture(m_weightTexture, atIndex: 1)
